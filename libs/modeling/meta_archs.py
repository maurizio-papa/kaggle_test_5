...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, reduction=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool1d(1)  # Output shape: B x C x 1
        self.max_pool = nn.AdaptiveMaxPool1d(1)  # Output shape: B x C x 1

        # MLP is implemented using Conv1d with kernel size 1
        self.fc1 = nn.Conv1d(in_planes, in_planes // reduction, 1, bias=False)
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Conv1d(in_planes // reduction, in_planes, 1, bias=False)

    def forward(self, x):
        avg_out = self.fc2(self.relu1(self.fc1(self.avg_pool(x))))
        max_out = self.fc2(self.relu1(self.fc1(self.max_pool(x))))
        out = avg_out + max_out
        return torch.sigmoid(out) * x  # Element-wise multiplication
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True)  # Output shape: B x 1 x T
        max_out, _ = torch.max(x, dim=1, keepdim=True)  # Output shape: B x 1 x T
        x = torch.cat([avg_out, max_out], dim=1)  # Concatenate along channel dimension, output shape: B x 2 x T
